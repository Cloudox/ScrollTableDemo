...
- (void)scrollBtnView {
    ...
    for (int i = 0; i < [self.btnTitleArray count]; i++) {
        UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(20 + i * 100, 0, 100, 25)];
        ...
        if (i != [self.btnTitleArray count]) {
            // 添加按钮间的分割线
            UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(118 + i*100, 0, 4, 25)];
            label.text = @"|";
            label.font = [UIFont systemFontOfSize:15];
            label.alpha = 0.3;
            [self.buttonScrollView addSubview:label];
        }
